...
const renameFile = () => {
    if (props.item) {
        ...
        renameForm.filename = props.item.filename.split(".")[0]
        // 保存文件后缀
        renameSuffix.value = '.' + props.item.filename.split(".")[1]
    ...
    await renameFormRef.value.validate((valid, fields) => {
        if (valid) {
            ...
            fileService.update({
                ...
                newFilename: renameForm.filename + renameSuffix.value
            }, res => {
                loading.value = false
